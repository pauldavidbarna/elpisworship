...
export const RESOURCES_UPDATED_EVENT = 'elpis-resources-updated';

// In-memory copy of the current data. Reads prefer this over localStorage
// so the app keeps working even if localStorage can't hold the payload.
let currentData: ResourcesData | null = null;
...
export function saveResourcesData(data: ResourcesData): void {
  ...
  try {
    ...
    // Quota errors are expected once photos/hero base64 fill the ~5MB budget.
    // Supabase is the source of truth — safe to skip the local cache write.
    if (!(err instanceof DOMException && err.name === 'QuotaExceededError')) {
      console.error('saveResourcesData localStorage error', err);
    } else {
      console.warn('localStorage quota exceeded; keeping data in memory only. Supabase remains source of truth.');
    }
  }
...
// An event is upcoming only if it's marked as such AND its date hasn't passed.
// Compare using start-of-day so an event happening today still counts as upcoming.
export function isUpcomingEvent(event: ResourceEvent, now: Date = new Date()): boolean {
...
export function isPastEvent(event: ResourceEvent, now: Date = new Date()): boolean {
