...
function PhotosAdmin({ data, onChange }: { data: ResourcesData; onChange: (d: ResourcesData) => void }) {
  ...
  // Images uploaded this session — deleted from storage on Cancel to avoid orphans.
  const addedRef = useRef<string[]>([]);
  // Images that existed on open but were removed — actually deleted from storage on Save.
  const removedRef = useRef<string[]>([]);

  const openAdd = () => {
    setEditing(null); setTitle(''); setImages([]);
    addedRef.current = []; removedRef.current = [];
    setOpen(true);
  };
  const openEdit = (g: PhotoGallery) => {
    setEditing(g); setTitle(g.title); setImages([...g.images]);
    addedRef.current = []; removedRef.current = [];
    setOpen(true);
  };
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    setUploading(true);
    try {
      const urls = await Promise.all(Array.from(files).map((f) => uploadPhoto(f)));
      addedRef.current.push(...urls);
  ...
  const removeImage = (idx: number) => {
    setImages((prev) => {
      const url = prev[idx];
      const addedIdx = addedRef.current.indexOf(url);
      if (addedIdx !== -1) {
        // Was uploaded this session and never persisted — delete from storage now.
        addedRef.current.splice(addedIdx, 1);
        deletePhoto(url).catch((e) => console.error('deletePhoto failed', e));
      } else {
        // Was already in the saved gallery — defer deletion to Save.
  ...
  const save = () => {
    // Commit deletions for images removed from an existing gallery.
    removedRef.current.forEach((url) => {
      deletePhoto(url).catch((e) => console.error('deletePhoto failed', e));
    });
    removedRef.current = [];
    addedRef.current = [];
  ...
  const cancel = () => {
    // Roll back this session's uploads so we don't orphan storage.
    addedRef.current.forEach((url) => {
      deletePhoto(url).catch((e) => console.error('deletePhoto failed', e));
    });
    addedRef.current = [];
    removedRef.current = [];
  ...
  const remove = (id: number) => {
    const gallery = data.photos.find((p) => p.id === id);
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={(v) => { if (!v) cancel(); else setOpen(v); }}>
...
function AnnouncementsAdmin({ data, onChange }: { data: ResourcesData; onChange: (d: ResourcesData) => void }) {
  ...
  const handleFile = async (files: FileList | null) => {
    ...
    try {
      ...
      // If the form already had a newly-uploaded image, remove it from storage.
  ...
  const save = () => {
    // If editing and the image was replaced, delete the previous one from storage.
  ...
  const remove = (id: number) => {
    ...
    if (a?.image) deletePhoto(a.image).catch(() => {});
    onChange({ ...data, announcements: data.announcements.filter((x) => x.id !== id) });
  };
...
function TeamAdmin({ data, onChange }: { data: ResourcesData; onChange: (d: ResourcesData) => void }) {
  ...
  const handleFile = async (files: FileList | null) => {
    ...
    setUploading(true);
    try {
      const url = await uploadPhoto(files[0], 600, 0.85);
      // Replace previous newly-uploaded image, if any, to avoid orphans.
      if (form.image && form.image !== editing?.image) {
        deletePhoto(form.image).catch(() => {});
      }
  ...
  const save = () => {
    // Delete the previous image from storage if it was replaced.
    if (editing && editing.image && editing.image !== form.image) {
      deletePhoto(editing.image).catch(() => {});
    }
    const updated = editing
  ...
  const remove = (id: number) => {
    ...
    if (m?.image) deletePhoto(m.image).catch(() => {});
    onChange({ ...data, team: data.team.filter((x) => x.id !== id) });
  };
...
function HeroAdmin({ data, onChange }: { data: ResourcesData; onChange: (d: ResourcesData) => void }) {
  ...
  const remove = (idx: number) => {
    const removed = data.heroImages[idx];
    if (removed) deletePhoto(removed.src).catch((e) => console.error('deletePhoto failed', e));
